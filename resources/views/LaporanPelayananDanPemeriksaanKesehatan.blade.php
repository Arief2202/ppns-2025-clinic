...
@section('body')
    @include('layouts.cardOpen')
        ...
        <div class="row mt-2 mb-3">
          <div class="col-6">
            <div class="col">
              <h5 class="card-title">Laporan Pelayanan Dan Pemeriksaan Kesehatan</h5>
            </div>
          </div>
                      ...
                      <table id="myTable">
                        ...
                        <tbody class="tbody">
                            ...
                            <tr>
                                ...
                                <td>{{date("d M Y", strtotime($data->tanggal_pelaporan))}}</td>
                                <td>{{$data->jenis_laporan}}</td>
                                <td><button class="btn btn-primary" onclick="window.open('{{$data->dokumen_laporan}}','_blank')">Lihat Dokumen</button></td>
                                <td>{{$data->editor()->name}}</td>
                                @if(Auth::user()->role_id == 4)
                                <td>
                                    @if(!$data->validator()) <form method="POST" action="/smk3/laporan-pelayanan-dan-pemeriksaan-kesehatan/validate">@csrf @endif
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="jenis_laporan" id="jenis_laporan" required>
                                    ...
                                    <option value="Laporan Pemeriksaan Kesehatan">Laporan Pemeriksaan Kesehatan</option>
                                </select>
                            </div>
                            <div class="mb-3">
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="/smk3/laporan-pelayanan-dan-pemeriksaan-kesehatan/edit" method="POST" enctype="multipart/form-data">@csrf
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="nama_laporanEdit" class="form-label">Nama Laporan</label>
                                <input type="text" class="form-control" id="nama_laporanEdit" name="nama_laporan">
                            </div>
                            <div class="mb-3">
                                <label for="tanggal_pelaporanEdit" class="form-label">Tanggal Pelaporan</label>
                                <input type="date" class="form-control" id="tanggal_pelaporanEdit" name="tanggal_pelaporan">
                            </div>
                            <div class="mb-3">
                                <label for="jenis_laporanEdit" class="form-label">Jenis Laporan</label>
                                <select class="form-select" name="jenis_laporan" id="jenis_laporanEdit">
                                    <option value="Laporan Pelayanan">Laporan Pelayanan</option>
                                    <option value="Laporan Pemeriksaan Kesehatan">Laporan Pemeriksaan Kesehatan</option>
                                </select>
                            </div>
    ...
    <script>
        ...
        function changeModal(id){
            ...
            xhttp.onload = function() {
                ...
                const datee = new Date(res.tanggal_pelaporan);
                ...
                document.getElementById('nama_laporanEdit').value=res.nama_laporan;
                document.getElementById('tanggal_pelaporanEdit').value= today;

                if(res.jenis_laporan == "Laporan Pelayanan") document.getElementById('jenis_laporanEdit').options[0].setAttribute('selected', true);
                else document.getElementById('jenis_laporanEdit').options[1].setAttribute('selected', true);
            }
            xhttp.open("GET", "/api/smk3/laporan-pelayanan-dan-pemeriksaan-kesehatan/get?id="+id, true);
            xhttp.send();
        }
        function del(){
            $.ajax({
                url: "/smk3/laporan-pelayanan-dan-pemeriksaan-kesehatan/delete",
                type:"POST",
                data:{
            ...
            setTimeout(function() {
                window.location.href = "/smk3/laporan-pelayanan-dan-pemeriksaan-kesehatan";
            }, 200);
        }
    </script>
@endsection
