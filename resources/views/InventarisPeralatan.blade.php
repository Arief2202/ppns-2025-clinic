...
@section('body')
                        ...
                        <tbody class="tbody">
                            ...
                            <tr>
                                ...
                                <td><button class="btn btn-primary"
                                        onclick="window.open('{{ $data->dokumen_inspeksi }}','_blank')">Lihat
                                        Dokumen</button></td>
                                <td>{{ date('d M Y', strtotime($data->tanggal_kalibrasi)) }}</td>
                                <td><button class="btn btn-primary"
                                        onclick="window.open('{{ $data->dokumen_kalibrasi }}','_blank')">Lihat
                                        Dokumen</button></td>
                                <td>{{ $data->editor()->name }}</td>
                                @if (Auth::user()->role_id == 1)
                                    <td>
                                        @if (!$data->validator())
                                            <form method="POST" action="/sarana-prasarana/inventaris-peralatan/validate">
                                                @csrf
                                        @endif
    ...
    <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="/sarana-prasarana/inventaris-peralatan/edit" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="tanggal_inspeksiEdit"
                                name="tanggal_inspeksi">
                        </div>
                        <div class="mb-3">
                            <label for="dokumen_inspeksiEdit" class="form-label">Dokumen Inspeksi</label>
                            <input type="file" class="form-control" id="dokumen_inspeksiEdit"
                                name="dokumen_inspeksi">
                        </div>
                        <div class="mb-3">
                            <label for="tanggal_kalibrasiEdit" class="form-label">Tanggal Kalibrasi</label>
                            <input type="date" class="form-control" id="tanggal_kalibrasiEdit"
                                name="tanggal_kalibrasi">
                        </div>
                        <div class="mb-3">
                            <label for="dokumen_kalibrasiEdit" class="form-label">Dokumen Kalibrasi</label>
                            <input type="file" class="form-control" id="dokumen_kalibrasiEdit"
                                name="dokumen_kalibrasi">
                        </div>
                    </div>
    ...
    <script>
        ...
        function changeModal(id) {
            ...
            xhttp.onload = function() {
                ...
                if (res.kategori_peralatan == "Medis") document.getElementById('kategori_peralatanEdit').options[0]
