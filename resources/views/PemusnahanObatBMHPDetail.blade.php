                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="mb-2">
                                    ...
                                    <input type="text" class="form-control" id="nomor_pengadaan" value="{{$data->pengadaan()->nomor_pengadaan}}" disabled>
                                </div>
                                <div class="mb-2">
                                    <label for="tanggal_pengadaan" class="form-label">Tanggal Pengadaan</label>
                                    <input type="text" class="form-control" id="tanggal_pengadaan" value="{{date("d M Y", strtotime($data->pengadaan()->tanggal_pengadaan))}}" disabled>
                                </div>
                                <div class="mb-2">
                                    <label for="detail_pengadaan" class="form-label">Detail Pengadaan</label><br>
                                    <button class="btn btn-secondary w-100" id="detail_pengadaan" onclick="window.open('/manajemen-farmasi/pengadaan/detail?id={{$data->pengadaan()->id}}','_blank')">Lihat Detail</button>
                                </div>
                                <div class="mb-2">
                            ...
                            <div class="col-6">
                                <div class="mb-2">
                                    ...
                                    <input type="text" class="form-control" id="tanggal_pemusnahan" value="{{date("d M Y", strtotime($data->tanggal_pemusnahan))}}" disabled>
                                </div>
                                <div class="mb-2">
                                    <label for="alasan_pemusnahan" class="form-label">Alasan Pemusnahan</label>
                                    <input type="text" class="form-control" id="alasan_pemusnahan" value="{{$data->alasan_pemusnahan}}" disabled>
                                </div>
                                <div class="mb-2">
                                        ...
                                        <div class="row">
                                            <div class="col-3">
                                                ...
                                                @if(!$data->validator()) <form method="POST" action="/manajemen-farmasi/pemusnahan/validate">@csrf @endif
                                                    ...
                                                    <button class="btn btn-success w-100" {{$data->validator() ? 'disabled' : 'type="submit"'}}>Validasi</button>
                                                @if(!$data->validator()) </form> @endif
                                            </div>
                                            <div class="col-9">
                                        @endif
                                                <label for="validator" class="form-label">Validator Pemusnahan</label>
                                                <input type="text" class="form-control" id="validator" value="{{$data->validator() ? $data->validator()->name : '-'}}" disabled>
                        ...
                        <table id="myTable">
                            ...
                            <tbody class="tbody">
                                ...
                                <tr>
                                    ...
                                    <td>{{$item->pemusnahan()->pengadaan()->items()->where('obat_bmhp_id', $item->obat_bmhp_id)->first()->jumlah}}</td>
                                    <td>
                                        <button class="btn @if($item->pemusnahan()->pengadaan()->items()->where('obat_bmhp_id', $item->obat_bmhp_id)->first()->hasExpired()) btn-danger @else btn-success @endif" disabled>
                                    ...
                                    <form method="POST" action="/manajemen-farmasi/pengadaan/detail/update">@csrf
                                        ...
                                        <td>
                                            @if(Auth::user()->role_id == 6)
                                            <input class="form-control" type="number" name="jumlah" max="{{$item->pemusnahan()->pengadaan()->items()->where('obat_bmhp_id', $item->obat_bmhp_id)->first()->jumlah}}" id="" value="{{$item->jumlah}}">
                                            @else
                                            {{$item->jumlah}}
                                            @endif
                                        </td>
                    ...
                    <form action="/manajemen-farmasi/pemusnahan/edit" method="POST" enctype="multipart/form-data">@csrf
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="tanggal_pemusnahanEdit" name="tanggal_pemusnahan" value="{{date("Y-m-d", strtotime($data->tanggal_pemusnahan))}}">
                            </div>
                            <div class="mb-3">
                                <label for="alasan_pemusnahanEdit" class="form-label">Alasan Pemusnahan</label>
                                <input type="text" class="form-control" id="alasan_pemusnahanEdit" name="alasan_pemusnahan" value="{{$data->alasan_pemusnahan}}">
                            </div>
                            <div class="mb-3">
